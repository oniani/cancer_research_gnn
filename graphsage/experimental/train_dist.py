    ...
    for epoch in range(args.num_epochs):
        ...
        for step, blocks in enumerate(dataloader):
            ...
            if step % args.log_every == 0:
                ...
                print(
                    "Epoch {:05d} | Step {:05d} | Loss {:.4f} | Train Acc {:.4f} | Speed (samples/sec) {:.4f} | GPU {:.1f} MiB | time {:.3f} s".format(
                        epoch,
                        step,
                        loss.item(),
                        acc.item(),
...
def main(args):
    ...
    g = dgl.distributed.DistGraph(
        args.ip_config, args.graph_name, conf_file=args.conf_path
    )
    print("rank:", g.rank())
    ...
    val_nid = dgl.distributed.node_split(
        g.ndata["val_mask"], g.get_partition_book(), force_even=True
    )
    test_nid = dgl.distributed.node_split(
        g.ndata["test_mask"], g.get_partition_book(), force_even=True
    )
    print(
        "part {}, train: {}, val: {}, test: {}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf_path", type=str, help="The path to the partition config file"
    )
