...
class NeighborSampler(object):
    ...
    def sample_blocks(self, seed_edges):
        ...
        if self.neg_share and n_edges % self.num_negs == 0:
            ...
            neg_tails = (
                neg_tails.view(-1, 1, self.num_negs)
                .expand(n_edges // self.num_negs, self.num_negs, self.num_negs)
                .flatten()
            )
            neg_heads = (
                heads.view(-1, 1).expand(n_edges, self.num_negs).flatten()
            )
        else:
...
def compute_acc(emb, labels, train_nids, val_nids, test_nids):
    ...
    pred = lr.predict(emb)
    f1_micro_eval = skm.f1_score(
        labels[val_nids], pred[val_nids], average="micro"
    )
    f1_micro_test = skm.f1_score(
        labels[test_nids], pred[test_nids], average="micro"
    )
    f1_macro_eval = skm.f1_score(
        labels[val_nids], pred[val_nids], average="macro"
    )
    f1_macro_test = skm.f1_score(
        labels[test_nids], pred[test_nids], average="macro"
    )
...
def run(proc_id, n_gpus, args, devices, data):
    ...
    if n_gpus > 0:
        ...
        train_seeds = train_seeds[
            proc_id * num_per_gpu : (proc_id + 1) * num_per_gpu
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (pos_graph, neg_graph, blocks) in enumerate(dataloader):
            ...
            if step % args.eval_every == 0 and proc_id == 0:
                ...
                print(
                    "Eval Acc {:.4f} Test Acc {:.4f}".format(
                        eval_acc, test_acc
                    )
                )
